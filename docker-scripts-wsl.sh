...
# Docker management scripts for Chemo Cursor project on WSL
...
check_wsl() {
    if ! grep -q "Microsoft" /proc/version 2>/dev/null; then
        print_warning "This script is optimized for WSL"
    else
...
check_docker() {
    if ! docker info > /dev/null 2>&1; then
        ...
        print_status "On WSL, you can start Docker with: sudo systemctl start docker"
        exit 1
    fi
}

# Function to optimize Docker for WSL
optimize_docker() {
    print_status "Optimizing Docker for WSL..."
...
EOF
    ...
    print_success "Docker optimized for WSL"
}

# Function to build and start production services
start_production() {
    print_status "Starting production services on WSL..."
    check_docker
    check_wsl
    check_resources
    ...
    print_success "Production services started successfully on WSL!"
...
start_development() {
    print_status "Starting development services on WSL..."
    check_docker
    check_wsl
    check_resources
    ...
    print_success "Development services started successfully on WSL!"
...
rebuild() {
    print_status "Rebuilding services for WSL..."
    docker-compose down
...
setup_wsl() {
    print_status "Setting up WSL environment..."
    ...
    print_success "WSL environment setup completed!"
    print_status "Please restart your WSL terminal or run: newgrp docker"
}
...
show_help() {
    ...
    echo ""
    echo "WSL Optimized Commands:"
    echo "  setup         Setup WSL environment (install Docker, etc.)"
    ...
    echo "  optimize      Optimize Docker for WSL"
    echo "  help          Show this help message"
    echo ""
    echo "Examples:"
    echo "  $0 setup             # Setup WSL environment"
